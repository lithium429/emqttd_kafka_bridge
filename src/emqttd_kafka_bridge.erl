...
on_message_publish(Message = #mqtt_message{pktid   = PkgId,
    ...
    io:format("print Payload is ~w~n", [Payload]),
    io:format("print Payload binary is ~w~n", [<<Payload/binary>>]),
    io:format("print topic is ~w~n", [Topic]),
    io:format("print topic binary is ~w~n", [<<Topic/binary>>]),
    Str1 = <<"{\"topic\":\"">>,
    Str2 = <<"\", \"message\":[\"">>,
    Str3 = <<"\"]}">>,
